            ...
            if (Spells.Smite != null)
            {
                    ...
                    .Where(
                        ...
                        && ((Spells.SummMenu["drake"].Cast<CheckBox>().CurrentValue
                            ...
                            || (Spells.SummMenu["red"].Cast<CheckBox>().CurrentValue
                                && jmob.BaseSkinName == "SRU_Red"))).Where(mob => smitemob).Where(mob => Player.Instance.GetSummonerSpellDamage(mob, DamageLibrary.SummonerSpells.Smite)
                                                                                                         >= mob.Health))
                {
                ...
                foreach (var target in ObjectManager.Get<AIHeroClient>()
                    .Where(
                        ...
                        && !Spells.SummMenu["DontSmite" + hero.BaseSkinName].Cast<CheckBox>().CurrentValue).Where(target => target.IsValidTarget(Spells.Smite.Range)))
                {
                    if (smitecombo)
                    {
