namespace AutoSteal
{
    ...
    internal class Check
    {
        ...
        public static List<Ti> TI = new List<Ti>();

        public Check()
        {
            var champs = ObjectManager.Get<AIHeroClient>().ToList();

            this.ATeam = champs.Where(x => x.IsAlly && x != null);
            this.ETeam = champs.Where(x => x.IsEnemy && x != null);

            Check.TI = this.ETeam.Select(x => new Ti(x)).ToList();
        ...
        private void Game_OnUpdate(EventArgs args)
        {
            ...
            foreach (var ti in Check.TI.Where(x => x.Player != null && x.Player.IsVisible && !x.Player.IsRecalling()))
            {
        ...
        {
                return Check.TI.Find(x => x.Player != null && x.Player.NetworkId == E.NetworkId);
        }

        public float GetTargetHealth(Ti ti, int addTime)
        {
            if (ti.Player.IsVisible && ti.Player != null)
            {
        ...
        public class HealthPrediction
        {
            ...
            private static readonly Dictionary<int, PredictedDamage> ActiveAttacks =
                new Dictionary<int, PredictedDamage>();
            ...
            private static void Game_OnGameUpdate(EventArgs args)
            {
                ActiveAttacks.ToList()
                    .Where(pair => pair.Value.StartTick < Helper.GameTimeTickCount - 3000)
                    .ToList()
            ...
            public static float GetHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
            {
                ...
                foreach (var attack in ActiveAttacks.Values)
                {
                    ...
                    if (!attack.Processed && attack.Source.IsValidTarget(float.MaxValue, false)
                        ...
                        && attack.Target.NetworkId == unit.NetworkId)
                    {
            ...
            public static float LaneClearHealthPrediction(Obj_AI_Base unit, int time, int delay = 70)
            {
                ...
                {
                    var n = 0;
                    if (Helper.GameTimeTickCount - 100 <= attack.StartTick + attack.AnimationTime
                        && attack.Target.IsValidTarget(float.MaxValue, false)
                        && attack.Source.IsValidTarget(float.MaxValue, false)
                        && attack.Target.NetworkId == unit.NetworkId)
                    {
                        var fromT = attack.StartTick;
                        var toT = Helper.GameTimeTickCount + time;

                        while (fromT < toT)
                        {
                            if (fromT >= Helper.GameTimeTickCount
                                && (fromT + attack.Delay
            ...
            {
                return
                    ActiveAttacks.Values.Any(m => (m.Source is Obj_AI_Minion) && m.Target.NetworkId == minion.NetworkId);
            }
            ...
            {
                return
                    ActiveAttacks.Values.Any(m => (m.Source is Obj_AI_Turret) && m.Target.NetworkId == minion.NetworkId);
            }
